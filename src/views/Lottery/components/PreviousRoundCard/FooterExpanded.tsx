...
const PreviousRoundCardFooter: React.FC<{ lotteryNodeData: LotteryRound; lotteryId: string }> = ({
  ...
  if (lotteryNodeData) {
    const { amountCollectedInBpt } = lotteryNodeData
    prizeInBusd = amountCollectedInBpt.times(cakePriceBusd)
  }
      ...
      <>
        ...
        {prizeInBusd.isNaN() ? (
          ...
          <Balance
            ...
            color="textSubtle"
            unit=" CAKE"
            value={getBalanceNumber(lotteryNodeData?.amountCollectedInBpt)}
            decimals={0}
          />
        )}
      </>
