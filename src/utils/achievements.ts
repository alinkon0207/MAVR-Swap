...
export const getAchievementDescription = (campaign: Campaign): TranslatableText => {
  switch (campaign.type) {
    case 'ifo':
      return {
        key: 'Committed more than $5 worth of LP in the %title% LAUNCHPAD',
        data: {
          title: campaign.title as string,
        },
      }
...
/**
 * Checks if a wallet is eligible to claim points from valid LAUNCHPAD's
 */
export const getClaimableIfoData = async (account: string): Promise<Achievement[]> => {
  const ifoCampaigns = ifosList.filter((ifoItem) => ifoItem.campaignId !== undefined)

  // Returns the claim status of every LAUNCHPAD with a campaign ID
  ...
  // Get LAUNCHPAD data for all LAUNCHPAD's that are eligible to claim
