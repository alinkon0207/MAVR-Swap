...
type PublicFarmData = {
  ...
  depositFee: string
  withdrawFee: string
  mavrPerBlock: SerializedBigNumber
}
...
const fetchFarm = async (farm: Farm): Promise<PublicFarmData> => {
  ...
  const [info, totalAllocPoint, mavrPerBlock] =
    pid || pid === 0
  ...
  const MavrPerBlock = mavrPerBlock ? new BigNumber(mavrPerBlock).div(BIG_TEN.pow(18)) : BIG_ZERO
  ...
  return {
    ...
    mavrPerBlock: MavrPerBlock.toJSON(),
  }
}
