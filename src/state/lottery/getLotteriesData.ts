...
const applyNodeDataToLotteriesGraphResponse = (
  ...
  if (graphResponse.length === 0) {
    return nodeData.map((nodeRound) => {
      console.log('debug res st2', nodeRound?.isLoading, nodeRound?.lotteryId, nodeRound?.status)
      return {
        ...
        ticketPrice: nodeRound.priceTicketInBpt,
        totalTickets: '',
        totalUsers: '',
        winningTickets: '',
      }
  ...
  const nodeRoundsWithGraphData = nodeData.map((nodeRoundData) => {
    ...
    console.log('debug node st1', nodeRoundData?.isLoading, nodeRoundData?.lotteryId, nodeRoundData?.status)
    return {
...
export const getGraphLotteries = async (
  ...
  try {
    ...
    console.log('debug graph', response.lotteries)
...
const getLotteriesData = async (currentLotteryId: string): Promise<LotteryRoundGraphEntity[]> => {
  ...
  console.log('debu lottery idx', idsForNodesCall)    // 461, 460, 459
  const nodeData = await fetchMultipleLotteries(idsForNodesCall)
  const graphResponse = await getGraphLotteries()
  console.log('debu lottery res st1', nodeData, graphResponse)
